...
def constraint(state, action):
    """The constraint function for the environment:
     C(s, a) = 1 if action a is available in state s, and 0 otherwise.
     This is used to determine whether an action is available or not in step().

     No-flip is possible unless every spin is up, i.e. state != [1,1,1,...]
     A given site can be flipped if the spin to the left is up, i.e. state[(action - 1)%L] == 1

    Args:
        state (_type_): _description_
        action (int): integer indicating the pair of sites to flip

    Returns:
        possible array: DeviceArray(value, dtype=bool)
    """

    cond_action_is_no_flip = jnp.array_equiv(action, len(state))
    # This indicates the do-nothing action for this environment

    def _pred_no_flip_is_possible(state, action):
        """Predicate for the condition for no-flip to occur. Returns 1 is no-flip possible, 0 otherwise."""
        must_flip = jnp.prod(jnp.equal(jnp.ones(len(state), dtype=state.dtype), state))
        return jnp.not_equal(must_flip, 1)

    def _pred_flip_chosen_is_possible(state, action):
        """Predicate for the condition that the chosen action is possible in this state. Returns 1 if possible, 0 otherwise."""
    ...
    possible = cond(
        ...
        _pred_no_flip_is_possible,
        _pred_flip_chosen_is_possible,
        state,
        action,
    )
